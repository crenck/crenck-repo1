...
#include "htmlactiontools.h"
#include "htmlwindowtools.h"
#include "htmltabtools.h"
#include "DebugLog.h"
...
SandstormApp::processCommandLine(int argc, QStringList argv)
{
    ...
    bool darkTheme = Prefs::Value(Prefs::FQ(p4v::prefs::DarkTheme),
                                  p4v::Preferences::Defaults()[p4v::prefs::DarkTheme].toBool());
    ...
    {
        rLog(0) << "Style sheet file '" << filename
                << "' not found. Defaulting to standard style." << qLogEnd;
        qWarning("error loading style sheet file");
    }
    ...
    if (mCommandLine->devjscript())
    {
        // HTTP Server start to support P4JScript development
        int httpPort = Prefs::Value(Prefs::FQ(p4v::prefs::HttpPort), p4v::Preferences::Defaults()[p4v::prefs::HttpPort].toInt());
        mHTTPServer.reset(new P4VHttp(httpPort));
        mRunHttp = mHTTPServer->isListening();
    }
    else
    {
        // start up the HTTP server for Visual Tools if Applets disabled.
        Gui::AppletPermissions gp;
        gp.restore();
        bool toolsEnabled = Prefs::Value(Prefs::FQ(p4v::prefs::HTMLToolsEnable), false);
        if (!gp.appletsAllowed() && toolsEnabled)
        {
            int httpPort = Prefs::Value(Prefs::FQ(p4v::prefs::HttpPort), p4v::Preferences::Defaults()[p4v::prefs::HttpPort].toInt());
            mHTTPServer.reset(new P4VHttp(httpPort));
            mRunHttp = mHTTPServer->isListening();
        ...
        if (!gp.appletsAllowed())
            Gui::Application::dApp->setToolsEnabled(toolsEnabled);
        else
            Gui::Application::dApp->setToolsEnabled(false);
    }
    ...
    if (mCommandLine->faceless())
    {
        ...
        int p4vcport = Platform::GetP4vcPort();
        mRCFServer->init("127.0.0.1", p4vcport);
        mRCFServer->setVersionString(Platform::ApplicationNameAndVersion());
        return;
    }
    ...
    if (darkTheme)
    {
        ...
        newPal.setColor(QPalette::Link, QColor(0, 137, 182));
        newPal.setColor(QPalette::LinkVisited, QColor(0, 137, 182));
        qApp->setPalette(newPal);
    }
    else
    {
        QPalette newPal(qApp->palette());
        newPal.setColor(QPalette::Link, Qt::blue);
        newPal.setColor(QPalette::LinkVisited, Qt::blue);
        qApp->setPalette(newPal);
    }
...
SandstormApp::Create()
{
    ...
    if (Prefs::Value(Prefs::FQ(p4v::prefs::HTMLToolsSoftwareRendering), false))
    {
    ...
    QString imageFilePath = Platform::ImageResourceFile(darkTheme);

    if (!QResource::registerResource(imageFilePath))
    {
    ...
    , mShortcutCmds(0)
{
    ...
    // Initialize icon scale from preferences to be used by IconManager
    IconManager::sIconScale = IconManager::IconScaleFactor();
    setAppIcon(IconManager::Pixmap(":/p4vimage.png"));
...
SandstormApp::init()
{
    ...
    // demo mode reset to false at application startup
    Prefs::SetValue(Prefs::FQ(p4v::prefs::HTMLToolsDemoMode), false);
...
SandstormApp::loadGlobalActions()
{
    ...
    if ( !Platform::IsMac() )
        a->setIcon( IconManager::Pixmap( ":/cmd_undo.png" ) );
    ...
    a->setMenuRole( QAction::NoRole );
    if ( !Platform::IsMac() )
        a->setIcon( IconManager::Pixmap( ":/cmd_redo.png" ) );
    ...
    a->setMenuRole(QAction::NoRole);
    if (!Platform::IsMac())
        a->setIcon(IconManager::Pixmap(":/cmd_init.png"));
...
SandstormApp::delayedInit()
{
    ...
    // setup WebEngine cookies and cache.
    bool htmlToolsEnabled = Prefs::Value( Prefs::FQ(p4v::prefs::HTMLToolsEnable), false);
    if ( htmlToolsEnabled )
        htmlToolsEnabled =  Prefs::Value(Prefs::FQ(p4v::prefs::features::HTMLTools),
                                         p4v::Preferences::Defaults()[p4v::prefs::features::HTMLTools].toBool());
    if ( htmlToolsEnabled )
    {
        QWebEngineProfile::defaultProfile()->setPersistentCookiesPolicy(QWebEngineProfile::AllowPersistentCookies);
        QString configdir = XMLManager::configDir();
        QString webeng = "webengine";
        QString cks = "cookies";
        QString cookiesdir = configdir + QDir::separator() + webeng + QDir::separator() + cks;
        QWebEngineProfile::defaultProfile()->setPersistentStoragePath(cookiesdir);
...
SandstormApp::startNewWorkspace(const Op::Configuration& cfg /* = Op::Configuration() */)
{
    ...
    while (true)
    {
        ...
        UIConfigurationDialog* dlg = new UIConfigurationDialog(pLastActiveWindow);
...
SandstormApp::createWorkspace(const Gui::WorkspaceScopeName& scopeName)
{
    ...
    if (mRunHttp)
    {
        mHTTPServer->appendWorkspace(ws);
    }
...
SandstormApp::customize(const QVariant&)
{
    ...
    if (win->exec() == QDialog::Accepted)
    {
        if (ws)
        {
            UIWorkspace2* workspace = dynamic_cast<UIWorkspace2*>(ws->windowManager()->mainWorkspaceWindow());
            if (workspace)
                workspace->updateAllToolsShortCuts();
        }
    }
}

void
SandstormApp::htmlActionsManager(const QVariant&)
{
    HTMLActionTools* win =
        new HTMLActionTools(qApp->activeWindow(),
                            winKey::CustomTools,
                            WinFlags::ApplicationModalFlags() );
    win->setWindowModality(Qt::ApplicationModal);
...
void
SandstormApp::htmlWindowsManager(const QVariant&)
{
    QDir dir = QDir::home();
    Gui::Workspace* ws = getActiveWorkspace();
    if (ws)
    ...
    HTMLWindowTools* win =
        new HTMLWindowTools(dir, qApp->activeWindow(),
                            winKey::CustomTools,
                            WinFlags::ApplicationModalFlags() );
    win->setWindowModality(Qt::ApplicationModal);
...
void
SandstormApp::htmlTabsManager(const QVariant& v)
{
    Q_UNUSED(v);
    ...
    TabCustomizeTools* win =
        new TabCustomizeTools(dir, qApp->activeWindow(),
                              winKey::CustomTools,
                              WinFlags::ApplicationModalFlags() );
    win->setWindowModality(Qt::ApplicationModal);
